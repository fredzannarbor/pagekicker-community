...
if [ "$summary" = "true" ] ; then
	/opt/bitnami/python/bin/python  $scriptpath"bin/wikifetcher.py" --infile "$TMPDIR$uuid/seeds/seedphrases" --outfile "$TMPDIR$uuid/wiki/wikiraw.md" --lang "$wikilocale" --summary 1> /dev/null
else
	/opt/bitnami/python/bin/python  $scriptpath"bin/wikifetcher.py" --infile "$TMPDIR$uuid/seeds/seedphrases" --outfile "$TMPDIR$uuid/wiki/wikiraw.md" --lang "$wikilocale" 1> /dev/null
fi
