...
if [ "$summary" = "true" ] ; then
	/opt/bitnami/python/bin/python  $scriptpath"bin/wikifetcher.py" --infile "$TMPDIR$uuid/seeds/sorted.seedfile" --outfile "$TMPDIR$uuid/wiki/wikiraw.md" --lang "$wikilang" --summary 1> /dev/null
else
	/opt/bitnami/python/bin/python  $scriptpath"bin/wikifetcher.py" --infile "$TMPDIR$uuid/seeds/sorted.seedfile" --outfile "$TMPDIR$uuid/wiki/wikiraw.md" --lang "$wikilang"  1> /dev/null
fi
...
if [ "$flickr" = "on" ] ; then
	...
	while read -r seed ; do
		python includes/Flickr_title_fetcher.py $TMPDIR$uuid/seeds/sorted.seedfile $TMPDIR$uuid/flickr/
		python includes/Flickr_seed_fetcher.py "$seed" $TMPDIR/$uuid/flickr/
	done<"$TMPDIR$uuid"/seeds/sorted.seedfile
else
...
cd $TMPDIR$uuid
/home/bitnami/.cabal/bin/pandoc -o "$TMPDIR$uuid/"$safe_product_name".epub" --epub-cover-image=$TMPDIR$uuid/cover/$sku"ebookcover.jpg" $TMPDIR$uuid/complete.md
/home/bitnami/.cabal/bin/pandoc -o "$TMPDIR$uuid/"$safe_product_name".docx"  $TMPDIR$uuid/complete.md
cd $scriptpath
