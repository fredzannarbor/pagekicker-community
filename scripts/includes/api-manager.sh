...
curl --silent -c cookies.txt -d "lgname="$WikipediaAPIuserID"&lgpassword="$WikipediaAPIuserPW"&action=login&format=xml" http://en.wikipedia.org/w/api.php -o output.xml
...
if [ $WikipediaAPIinUse = "1" ] ; then
	...
	curl --silent -b cookies.txt -d "action=login&lgname="$WikipediaAPIuserID"&lgpassword="$WikipediaAPIuserPW"&format=xml&lgtoken="$TOKEN http://en.wikipedia.org/w/api.php 
	echo " "
else
